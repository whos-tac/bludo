defmodule Bludo.Presentations do
  @moduledoc """
  ...
  When a `Bludo.Presentations.PresentationFile{}` struct is provided, the
  ...
  def get_slide_urls(hash, length) when is_binary(hash) and is_integer(length) do
    config = Application.get_env(:bludo, :presentations)
  ...
  def subscribe(presentation_file_id) do
    Phoenix.PubSub.subscribe(Bludo.PubSub, "presentation:#{presentation_file_id}")
  end
