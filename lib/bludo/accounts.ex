defmodule Bludo.Accounts do
  ...
  alias Bludo.Accounts.{User, UserToken, UserNotifier, Role}
  ...
  def remove_oidc_user(bludo_user, issuer) do
    Repo.delete_all(
      from u in Accounts.Oidc.User,
        where: u.issuer == ^issuer and u.user_id == ^bludo_user.id
    )
  end
  ...
  defp create_new_user(attrs) do
    with {:ok, bludo_user} <- get_user_by_email_or_create(attrs.email),
         updated_attrs <-
           Map.merge(attrs, %{user_id: bludo_user.id}),
         {:ok, user} <- create_oidc_user(updated_attrs) do
      {:ok, user |> Repo.preload(:user)}
    else
      _ -> {:error, %{reason: :invalid_user, msg: "Invalid bludo user"}}
    end
  end
