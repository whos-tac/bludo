defmodule Bludo.Events do
  ...
  alias Bludo.{Accounts, Presentations, Repo}
      ...
      polls when is_list(polls) ->
        polls =
          for poll <- polls do
            ...
            {:ok, poll} = Bludo.Polls.create_poll(attrs)
            poll
          end
      ...
      forms when is_list(forms) ->
        forms =
          for form <- forms do
            ...
            {:ok, form} = Bludo.Forms.create_form(attrs)
            form
          end
      ...
      embeds when is_list(embeds) ->
        embeds =
          for embed <- embeds do
            ...
            {:ok, embed} = Bludo.Embeds.create_embed(attrs)
            embed
          end
      ...
      quizzes when is_list(quizzes) ->
        quizzes =
          for quiz <- quizzes do
            ...
            {:ok, quiz} = Bludo.Quizzes.create_quiz(attrs)
            quiz
          end
  ...
  defp broadcast_event(event_uuid, message) when is_binary(event_uuid) do
    Phoenix.PubSub.broadcast(Bludo.PubSub, "event:#{event_uuid}", message)
  end
  ...
  def subscribe_user_events(user_id) when is_integer(user_id) do
    Phoenix.PubSub.subscribe(Bludo.PubSub, "user:#{user_id}:events")
  end

  def broadcast_user_events(user_id, message) when is_integer(user_id) do
    Phoenix.PubSub.broadcast(Bludo.PubSub, "user:#{user_id}:events", message)
  end
