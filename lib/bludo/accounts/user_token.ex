defmodule Bludo.Accounts.UserToken do
  @moduledoc """
  ...
  def build_session_token(user) do
    ...
    {token, %Bludo.Accounts.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Bludo.Accounts.UserToken, where: [token: ^token, context: ^context]
  end
  ...
  def user_and_contexts_query(user, :all) do
    from t in Bludo.Accounts.UserToken, where: t.user_id == ^user.id
  end

  def user_and_contexts_query(user, [_ | _] = contexts) do
    from t in Bludo.Accounts.UserToken, where: t.user_id == ^user.id and t.context in ^contexts
  end

  def user_magic_and_contexts_query(email, [_ | _] = contexts) do
    from t in Bludo.Accounts.UserToken, where: t.sent_to == ^email and t.context in ^contexts
  end

  def user_magic_and_contexts_expiry_query(email) do
    from t in Bludo.Accounts.UserToken,
      where:
