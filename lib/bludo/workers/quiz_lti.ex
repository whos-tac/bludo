defmodule Bludo.Workers.QuizLti do
  ...
  def perform(%Oban.Job{args: %{"action" => "create", "quiz_id" => quiz_id}}) do
    with quiz <- Bludo.Quizzes.get_quiz!(quiz_id),
         presentation_file <-
  ...
  def perform(%Oban.Job{
    ...
    with quiz <- Bludo.Quizzes.get_quiz!(quiz_id, [:lti_resource]),
         user <- Bludo.Accounts.get_user!(user_id) |> Bludo.Repo.preload(:lti_user) do
