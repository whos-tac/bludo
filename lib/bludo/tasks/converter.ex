defmodule Bludo.Tasks.Converter do
  @moduledoc """
  ...
  def convert(user_id, file, hash, ext, presentation_file_id, is_copy \\ false) do
    presentation = Bludo.Presentations.get_presentation_file!(presentation_file_id, [:event])

    {:ok, presentation} =
      Bludo.Presentations.update_presentation_file(presentation, %{
        "status" => "progress"
      })
  ...
  defp get_presentation_storage do
    Application.get_env(:bludo, :presentations) |> Keyword.get(:storage)
  end

  defp get_presentation_storage_dir do
    Application.get_env(:bludo, :storage_dir)
  end

  defp get_s3_bucket do
    Application.get_env(:bludo, :presentations) |> Keyword.get(:s3_bucket)
  end

  defp get_resolution do
    Application.get_env(:bludo, :presentations) |> Keyword.get(:resolution)
  end
