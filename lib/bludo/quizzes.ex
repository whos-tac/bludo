defmodule Bludo.Quizzes do
  ...
  alias Bludo.Quizzes.QuizQuestion
  alias Bludo.Quizzes.QuizQuestionOpt
  alias Bludo.Quizzes.QuizResponse
    ...
    |> case do
      {:ok, quiz} ->
        if attrs["lti_resource_id"] do
          Bludo.Workers.QuizLti.create(quiz.id) |> Oban.insert()
        end

        presentation_file =
          Bludo.Presentations.get_presentation_file!(quiz.presentation_file_id, [
            :event
          ])
    ...
    |> case do
      {:ok, updated_quiz} ->
        if quiz.lti_resource_id do
          Bludo.Workers.QuizLti.edit(quiz.id) |> Oban.insert()
        end
