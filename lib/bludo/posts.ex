defmodule Bludo.Posts do
  ...
  def reacted_posts(event_id, user_id, icon) when is_number(user_id) do
    from(reaction in Bludo.Posts.Reaction,
      join: post in Bludo.Posts.Post,
  ...
  alias Bludo.Posts.{Reaction, Post}
  ...
  defp broadcast({:ok, post}, event) do
    Phoenix.PubSub.broadcast(Bludo.PubSub, "event:#{post.event.uuid}", {event, post})
    {:ok, post}
  end
end
