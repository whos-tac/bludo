defmodule BludoWeb.UserAuth do
  @moduledoc """
  ...
  import BludoWeb.Helpers.ConnUtils, only: [get_client_ip: 1, get_user_agent: 1]

  alias Bludo.{Accounts, Audit}
  ...
  def log_out_user(conn) do
    logout_redirect_url = Application.get_env(:bludo, :logout_redirect_url)
    user_token = get_session(conn, :user_token)
    user_token && Accounts.delete_session_token(user_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      BludoWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  defp async_log_action(user, action, metadata) do
    Task.Supervisor.start_child(Bludo.TaskSupervisor, fn ->
