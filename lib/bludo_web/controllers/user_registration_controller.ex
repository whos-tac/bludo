defmodule BludoWeb.UserRegistrationController do
  ...
  def new(conn, _params) do
    if Application.get_env(:bludo, :enable_account_creation) do
  ...
  def create(conn, %{"user" => user_params}) do
    case Accounts.register_user(user_params(user_params)) do
      {:ok, user} ->
        if Application.get_env(:bludo, :email_confirmation) do
          {:ok, _} =
