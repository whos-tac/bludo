defmodule BludoWeb.UserSessionController do
  ...
  def new(conn, _params) do
    oidc_auto_redirect_login = Application.get_env(:bludo, :oidc)[:auto_redirect_login]
  ...
  defp redirect_to_login(conn, false) do
    oidc_provider_name = Application.get_env(:bludo, :oidc)[:provider_name]
    oidc_logo_url = Application.get_env(:bludo, :oidc)[:logo_url]
    oidc_enabled = Application.get_env(:bludo, :oidc)[:enabled]
  ...
  def create(conn, %{"user" => user_params}) do
    ...
    if user = Accounts.get_user_by_email_and_password(email, password) do
      if Application.get_env(:bludo, :email_confirmation) and !user.confirmed_at do
