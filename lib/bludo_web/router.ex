defmodule BludoWeb.Router do
  use BludoWeb, :router

  import BludoWeb.{UserAuth, EventController}

  pipeline :browser do
    plug(:accepts, ["html"])
    plug(:fetch_session)
    plug(:fetch_live_flash)
    plug(:put_root_layout, html: {BludoWeb.LayoutView, :root})
    plug(:protect_from_forgery)
    plug(:put_secure_browser_headers)
    plug(:fetch_current_user)
    plug(BludoWeb.Plugs.Locale)
  end

  pipeline :admin_required do
    plug(BludoWeb.Plugs.AdminRequiredPlug)
  end
  ...
  pipeline :rate_limit_auth do
    plug BludoWeb.Plugs.RateLimitPlug,
      max_requests: 10,
      interval_ms: 60_000,
      prefix: "auth"
  end
  ...
  live_session :attendee do
    scope "/", BludoWeb do
      pipe_through([:browser, :attendee_registration, BludoWeb.Plugs.Iframe])
  ...
  live_session :user,
    root_layout: {BludoWeb.LayoutView, :user} do
    scope "/", BludoWeb do
  ...
  live_session :presenter, on_mount: BludoWeb.UserLiveAuth do
    scope "/", BludoWeb do
      pipe_through([:browser, :require_authenticated_user])
  ...
  if Mix.env() in [:dev, :test] do
    ...
    scope "/" do
      ...
      live_dashboard("/dashboard", metrics: BludoWeb.Telemetry)
    end
  end
  ...
  live_session :admin, root_layout: {BludoWeb.LayoutView, :admin} do
    scope "/admin", BludoWeb.AdminLive do
