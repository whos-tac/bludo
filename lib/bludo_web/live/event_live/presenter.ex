defmodule BludoWeb.EventLive.Presenter do
  ...
  def mount(%{"code" => code} = params, session, socket) do
    ...
    if is_nil(event) || not leader?(socket, event) do
      ...
      endpoint_config = Application.get_env(:bludo, BludoWeb.Endpoint)[:url]
  ...
  def handle_info(
    ...
    send_update(
      BludoWeb.EventLive.ManageableQuizComponent,
  ...
  defp poll_at_position(%{assigns: %{event: event, state: state}} = socket) do
    with poll <-
           Bludo.Polls.get_poll_current_position(
  ...
  defp form_at_position(%{assigns: %{event: event, state: state}} = socket) do
    with form <-
           Bludo.Forms.get_form_current_position(
             event.presentation_file.id,
             state.position
           ) do
  ...
  defp embed_at_position(%{assigns: %{event: event, state: state}} = socket) do
    with embed <-
           Bludo.Embeds.get_embed_current_position(
             event.presentation_file.id,
             state.position
           ) do
  ...
  defp quiz_at_position(%{assigns: %{event: event, state: state}} = socket) do
    with quiz <-
           Bludo.Quizzes.get_quiz_current_position(
             event.presentation_file.id,
             state.position
           ) do
