defmodule BludoWeb.EventLive.FormComponent do
              ...
              <%= for field <- @form.fields do %>
                <%= case field.type do %>
                  <% "text" -> %>
                    ...
                    />
                  <% "email" -> %>
                    <BludoWeb.Component.Input.email
                      form={f}
                      labelClass="text-white"
                      fieldClass="bg-gray-700 text-white"
                      key={safe_field_atom(field.name)}
  ...
  def handle_event("validate", %{"form_submit" => form_submit_params}, socket) do
    changeset =
      (socket.assigns.current_form_submit || %Bludo.Forms.FormSubmit{})
      |> Bludo.Forms.change_form_submit(form_submit_params)
  ...
  def handle_event(
    ...
    case Bludo.Forms.create_or_update_form_submit(
