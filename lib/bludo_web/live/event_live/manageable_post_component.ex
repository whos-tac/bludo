defmodule BludoWeb.EventLive.ManageablePostComponent do
  ...
  def render(assigns) do
    ...
    <div
      id={"#{@id}"}
      class={"#{if BludoWeb.Helpers.body_without_links(@post.body) =~ "?", do: "border-supporting-yellow-400 border-2"} flex flex-col md:block px-4 pb-2 pt-3 rounded-b-lg rounded-tr-lg bg-white relative shadow-md text-black break-all mt-2"}
    >
      <div
        :if={BludoWeb.Helpers.body_without_links(@post.body) =~ "?"}
      ...
      <div class="flex space-x-3 items-center">
        ...
        <div class="flex flex-col">
          ...
          <p class="text-xl">
            {BludoWeb.Helpers.format_body(@post.body)}
          </p>
        </div>
      </div>
