defmodule BludoWeb.EventLive.Show do
  alias Bludo.Interactions
  use BludoWeb, :live_view

  alias Bludo.{Posts, Polls, Forms, Quizzes, Stats}
  alias BludoWeb.Presence

  on_mount(BludoWeb.AttendeeLiveAuth)
  ...
  defp leader_list(socket, event) do
    assign(socket, :leaders, Bludo.Events.get_activity_leaders_for_event(event.id))
  end

  defp maybe_update_audience_peak(event, online) do
    if online > event.audience_peak do
      Bludo.Events.update_event(event, %{audience_peak: online})
    end
  end
  ...
  defp check_leader(%{assigns: %{current_user: current_user} = _assigns} = socket, event)
       when is_map(current_user) do
    is_leader =
      current_user.id == event.user_id || Bludo.Events.led_by?(current_user.email, event)
  ...
  defp starting_soon_assigns(socket, event) do
    if Bludo.Events.Event.started?(event) do
  ...
  @impl true
  def handle_info({:poll_updated, %Bludo.Polls.Poll{enabled: true} = poll}, socket) do
    {:noreply,
     socket
  ...
  @impl true
  def handle_info({:poll_deleted, %Bludo.Polls.Poll{enabled: true}}, socket) do
    {:noreply,
     socket
  ...
  @impl true
  def handle_info({:form_updated, %Bludo.Forms.Form{enabled: true} = form}, socket) do
    {:noreply,
     socket
  ...
  @impl true
  def handle_info({:form_deleted, %Bludo.Forms.Form{enabled: true}}, socket) do
    {:noreply,
     socket
     |> update(:current_interaction, fn _current_interaction -> nil end)}
  end
  ...
  @impl true
  def handle_info({:embed_updated, %Bludo.Embeds.Embed{enabled: true} = embed}, socket) do
    {:noreply,
     socket
  ...
  @impl true
  def handle_info({:embed_deleted, %Bludo.Embeds.Embed{enabled: true}}, socket) do
    {:noreply,
     socket
     |> update(:current_interaction, fn _current_interaction -> nil end)}
  end
  ...
  @impl true
  def handle_info({:quiz_updated, %Bludo.Quizzes.Quiz{enabled: true} = quiz}, socket) do
    {:noreply,
     socket
  ...
  @impl true
  def handle_info({:quiz_deleted, %Bludo.Quizzes.Quiz{enabled: true}}, socket) do
    {:noreply,
     socket
     |> update(:current_interaction, fn _current_interaction -> nil end)}
  end
  ...
  def handle_event(
      ...
      when is_map(current_user) do
    case Bludo.Quizzes.submit_quiz(
           current_user,
           socket.assigns.event.uuid,
           opts,
           socket.assigns.current_interaction.id
