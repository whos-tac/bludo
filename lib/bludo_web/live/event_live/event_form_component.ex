defmodule BludoWeb.EventLive.EventFormComponent do
      ...
      {:ok, event} ->
        with e <- Events.get_event!(event.uuid, [:leaders]) do
          Enum.each(e.leaders, fn leader ->
            Bludo.Accounts.LeaderNotifier.deliver_event_invitation(
              e.name,
              leader.email,
              url(~p"/events")
            )
  ...
  defp handle_file_conversion(socket, hash, ext, event) do
    if is_nil(hash) || is_nil(ext) do
      ...
      files = Bludo.Presentations.get_presentation_files_by_hash(event.presentation_file.hash)
  ...
  defp get_max_file_size() do
    Application.get_env(:bludo, :presentations) |> Keyword.get(:max_file_size)
  end
