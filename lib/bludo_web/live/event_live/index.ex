defmodule BludoWeb.EventLive.Index do
  use BludoWeb, :live_view

  alias Bludo.{Events, Presentations}
  ...
  def handle_event("validate", %{"event" => event_params}, socket) do
    changeset =
      %Event{}
      |> Bludo.Events.change_event(event_params)
  ...
  def handle_event("save", %{"event" => event_params}, socket) do
    ...
    case Bludo.Events.create_event(
           event_params
  ...
  def handle_event("delete", %{"id" => id}, %{assigns: %{current_user: current_user}} = socket) do
    ...
    files =
      Bludo.Presentations.get_presentation_files_by_hash(hash)

    {:ok, _} = Events.delete_event(event)

    if files |> Enum.empty?() && !is_nil(hash) do
      Task.Supervisor.async_nolink(Bludo.TaskSupervisor, fn ->
        Bludo.Tasks.Converter.clear(event.presentation_file.hash)
      end)
    end
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    if event.expired_at && NaiveDateTime.compare(NaiveDateTime.utc_now(), event.expired_at) == :gt do
      ...
      if event.presentation_file.status == "fail" && event.presentation_file.hash do
        Bludo.Presentations.update_presentation_file(event.presentation_file, %{
          "status" => "done"
        })
      end
