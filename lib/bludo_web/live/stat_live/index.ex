defmodule BludoWeb.StatLive.Index do
  use BludoWeb, :live_view

  alias Bludo.Events

  on_mount(BludoWeb.UserLiveAuth)
  ...
  def mount(%{"id" => id}, session, socket) do
    ...
    event = %{
      ...
      | presentation_file: %{
          ...
          | quizzes: Enum.map(event.presentation_file.quizzes, &Bludo.Quizzes.set_percentages/1)
        }
    }

    distinct_attendee_count = Bludo.Stats.get_unique_attendees_for_event(event.id)
    distinct_poster_count = Bludo.Stats.distinct_poster_count(event.id)
  ...
  defp average_polls(event, unique_attendees) do
    poll_ids = Bludo.Polls.list_polls(event.presentation_file.id) |> Enum.map(& &1.id)
    ...
    case poll_ids do
      ...
      poll_ids ->
        distinct_votes = Bludo.Stats.get_distinct_poll_votes(poll_ids)
  ...
  defp average_quizzes(event, unique_attendees) do
    quiz_ids = Bludo.Quizzes.list_quizzes(event.presentation_file.id) |> Enum.map(& &1.id)
    ...
    case quiz_ids do
      ...
      quiz_ids ->
        distinct_votes = Bludo.Stats.get_distinct_quiz_responses(quiz_ids)
  ...
  defp average_forms(event, unique_attendees) do
    form_ids = Bludo.Forms.list_forms(event.presentation_file.id) |> Enum.map(& &1.id)
    ...
    case form_ids do
      ...
      form_ids ->
        distinct_submits = Bludo.Stats.get_distinct_form_submits(form_ids)
